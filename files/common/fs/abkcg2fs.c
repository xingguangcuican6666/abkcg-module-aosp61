...
static const struct inode_operations abkcg2fs_file_iops;
...
static struct inode *abkcg2fs_new_inode(struct super_block *sb, umode_t mode)
{
	...
	if (S_ISDIR(mode)) {
		...
		inode->i_op = &abkcg2fs_file_iops;
		inode->i_fop = &abkcg2fs_file_fops;
	}
	return inode;
}

static int abkcg2fs_file_setattr(struct user_namespace *mnt_userns,
				 struct dentry *dentry, struct iattr *attr)
{
	/*
	 * Control files are virtual and always zero-sized. Redirections such as
	 * "echo 2 > pids.max" arrive with O_TRUNC, so normalize size changes to
	 * zero instead of letting the VFS walk an unsupported truncate path.
	 */
			  ...
			  struct dentry *dentry, umode_t mode)
{
	...
	ret = vfs_path_lookup(parent->lower_path.dentry, parent->lower_path.mnt,
			      dentry->d_name.name, LOOKUP_FOLLOW | LOOKUP_DIRECTORY,
			      &lower_path);
	if (ret) {
		dput(child);
		inode_unlock(lower_dir);
